...
public class StudySignupController {
    ...
    public String signUp(StudySignUpForm form, Model model) {
        final User student = User.builder()
                            ...
                            .enabled(true)
                            .build();
        schoolService.findSchool(form.getSchoolId()).ifPresent(school -> student.setSchool(school));
        userService.findUser(form.getTeacherId()).ifPresent(teacher -> student.setTeacher(teacher));

        User savedUser = userService.save(student);
        userService.addAuthority(savedUser.getUserId(), Authority.ROLE_STUDENT);
        model.addAttribute("site", "study");
